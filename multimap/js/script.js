...
if (Modernizr.webgl) {
  ...
  function ready(error, data, config, geog) {
    ...
    lastdate=d3.timeFormat("%d/%m/%Y")(d3.max(dates));
    ...
    function defineBreaks() {
      ...
      if (config.ons.breaks == "jenks" || config.ons.breaks == "equal") {
        var values = filter.map(function(d) {
          return +d[lastdate];
        }).filter(function(d) {
          return !isNaN(d);
        }).sort(d3.ascending);
    ...
    function updateLayers() {
      ...
      areas.features.map(function(d, i) {
        if (!isNaN(rateById[d.properties.AREACD])) {
          ...
          d.properties.fill = config.ons.nullcolour
        };
